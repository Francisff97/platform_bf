<x-admin-layout title="Create Builder">
  ...
  <form method="POST" action="{{ route('admin.builders.store') }}" enctype="multipart/form-data"
    ...
    <div>
      ...
      <input name="name" placeholder="Builder name" value="{{ old('name') }}"
    ...
    <div>
      ...
      <input name="team" placeholder="Team / organization" value="{{ old('team') }}"
             class="h-11 w-full rounded-xl border border-[color:var(--accent)] bg-white/90 px-3 text-black placeholder:text-gray-500 outline-none transition
                    hover:border-[color:var(--accent)]/80 focus:ring-2 focus:ring-[color:var(--accent)]
                    dark:bg-black/80 dark:text-white dark:placeholder:text-gray-400" />
    </div>
    ...
    <div>
      ...
      <textarea name="skills" rows="3" placeholder="Comma separated (e.g. TH16, Anti-3, Hybrid)"
                class="w-full rounded-xl border border-[color:var(--accent)] bg-white/90 p-3 text-black placeholder:text-gray-500 outline-none transition
                       hover:border-[color:var(--accent)]/80 focus:ring-2 focus:ring-[color:var(--accent)]
                       dark:bg-black/80 dark:text-white dark:placeholder:text-gray-400">{{ old('skills') }}</textarea>
    </div>

    {{-- Description --}}
    <div>
      <label class="mb-1 block text-sm font-medium text-gray-800 dark:text-gray-200">Description</label>
      <textarea name="description" rows="6" placeholder="Bio / description of the builder…"
                class="w-full rounded-xl border border-[color:var(--accent)] bg-white/90 p-3 text-black placeholder:text-gray-500 outline-none transition
                       hover:border-[color:var(--accent)]/80 focus:ring-2 focus:ring-[color:var(--accent)]
                       dark:bg-black/80 dark:text-white dark:placeholder:text-gray-400">{{ old('description') }}</textarea>
    </div>
