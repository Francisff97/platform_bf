<x-admin-layout title="Coaches">
      ...
      <div class="overflow-hidden rounded-xl border bg-white shadow-sm ring-1 ring-black/5 dark:border-gray-800 dark:bg-gray-900 dark:ring-white/10">
        <div class="flex items-start gap-3 p-3">
          ...
          <div class="h-12 w-12 overflow-hidden rounded-full bg-gray-100 ring-1 ring-black/5 dark:bg-gray-800 dark:ring-white/10">
            @if($c->image_path)
              <img src="{{ Storage::url($c->image_path) }}" class="h-full w-full object-cover" alt="">
            @endif
          </div>
          <div class="min-w-0 flex-1">
            <div class="line-clamp-1 font-medium text-gray-900 dark:text-gray-100">{{ $c->name }}</div>
            <div class="text-xs text-gray-500 dark:text-gray-400">{{ $c->team ?? '—' }}</div>
            @if($c->skills)
              <div class="mt-1 line-clamp-1 text-xs text-gray-600 dark:text-gray-300">
                {{ is_array($c->skills) ? implode(', ', $c->skills) : $c->skills }}
              </div>
            @endif
          </div>
        </div>
        <div class="flex items-center justify-end gap-3 border-t p-3 text-sm dark:border-gray-800">
          <a class="text-indigo-600 hover:underline" href="{{ route('admin.coaches.edit',$c) }}">Edit</a>
        ...
        @foreach($coaches as $c)
          <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/60">
            ...
            <td class="px-4 py-3 text-gray-600 dark:text-gray-300">
              @if($c->skills) {{ is_array($c->skills) ? implode(', ', $c->skills) : $c->skills }} @else — @endif
            </td>
  ...
  @if(method_exists($coaches,'links'))
