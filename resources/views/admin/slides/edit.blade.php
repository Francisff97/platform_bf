<x-admin-layout title="Edit slide">
  ...
  @if ($errors->any())
    <div class="mb-4 rounded-xl border border-red-300 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-200">
      <div class="mb-1 font-semibold">Fix these errors:</div>
  ...
  <form method="POST" action="{{ route('admin.slides.update',$slide) }}" enctype="multipart/form-data"
    ...
    <div class="rounded-2xl border bg-white/70 p-5 shadow-sm ring-1 ring-black/5 backdrop-blur
      ...
      <div class="mt-4 grid gap-4 md:grid-cols-2">
        <label class="block">
          ...
          <input name="cta_label" value="{{ old('cta_label',$slide->cta_label) }}"
                 class="h-11 w-full rounded-xl border px-3 dark:border-gray-700 dark:bg-gray-900 dark:text-white" />
        </label>
        <label class="block">
          <div class="mb-1 text-sm font-medium text-gray-700 dark:text-gray-200">CTA URL</div>
          <input name="cta_url" value="{{ old('cta_url',$slide->cta_url) }}"
                 class="h-11 w-full rounded-xl border px-3 dark:border-gray-700 dark:bg-gray-900 dark:text-white" />
        </label>
      </div>
    </div>
    ...
    <div class="rounded-2xl border bg-white/70 p-5 shadow-sm ring-1 ring-black/5 backdrop-blur
                dark:border-gray-800 dark:bg-gray-900/70 dark:ring-white/10"
         x-data="{ preview: '{{ $slide->image_path ? e(Storage::url($slide->image_path)) : '' }}' }">
      <div class="flex flex-col gap-4 md:flex-row md:items-center">
        <div class="w-full md:w-1/2">
          ...
          @if($slide->image_path)
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Current: {{ $slide->image_path }}</p>
          @endif
        </div>
        ...
        <div class="w-full md:w-1/2">
          ...
          <div class="relative aspect-[16/9] w-full overflow-hidden rounded-xl ring-1 ring-black/5 dark:ring-white/10">
            ...
            <div x-show="preview" class="pointer-events-none absolute inset-0 bg-gradient-to-b from-black/40 via-black/20 to-black/40"></div>
            <div x-show="preview" class="pointer-events-none absolute bottom-3 left-3 inline-flex items-center gap-2 rounded-full bg-[var(--accent)] px-3 py-1.5 text-xs font-medium text-white">
              {{ $slide->cta_label ?: 'CTA' }}
    ...
    <div class="flex items-center gap-3">
      <button class="inline-flex items-center rounded-xl bg-[color:var(--accent)] px-4 py-2.5 text-white hover:opacity-90">Update</button>
