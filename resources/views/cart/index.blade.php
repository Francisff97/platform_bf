  ...
  @else
        ...
        <div class="rounded-2xl border bg-white/70 p-3 shadow-sm backdrop-blur
          ...
          <div class="mt-3 flex items-center justify-end gap-2">
            <form method="POST" action="{{ route('cart.remove',$i) }}">@csrf
              <button class="rounded-lg px-3 py-1.5 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">
                Remove
              </button>
            </form>
          </div>
    ...
    <div class="mt-4 flex flex-col-reverse items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between">
      <form method="POST" action="{{ route('cart.clear') }}">@csrf
        <button class="rounded-lg px-3 py-2 text-sm opacity-80 ring-1 ring-black/10 hover:bg-black/5 dark:ring-white/10 dark:hover:bg-white/10">
          Empty cart
        </button>
      </form>
      ...
      <div class="flex items-center justify-between gap-3 sm:justify-end">
        ...
        <a href="{{ route('checkout.index') }}"
           class="rounded-xl bg-[var(--accent)] px-5 py-2.5 text-white shadow-sm ring-1 ring-white/10 hover:opacity-95">
          Go to checkout
        </a>
      </div>
    </div>
