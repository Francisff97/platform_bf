<x-app-layout>
        ...
        <div class="mt-4">
          ...
          <div class="mt-2 flex flex-wrap gap-2">
            @foreach($builder->skills as $s)
              <span class="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-medium dark:bg-gray-800 dark:text-gray-100">{{ $s }}</span>
            @endforeach
          </div>
        </div>
      @endif
  ...
  <section class="mx-auto max-w-6xl px-4 pb-12">
    <h2 class="mb-3 mt-4 text-lg font-semibold text-gray-900 dark:text-gray-100">Packs by {{ $builder->name }}</h2>
    @if($packs->count())
