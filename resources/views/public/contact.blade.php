...
<x-app-layout>
  ...
  {{-- Helpers visual senza wallpaper --}}
  <style>
    ...
    .ring-gradient{ position:relative; border-radius:22px; padding:1px;
    ...
    .fl textarea:focus + label, .fl textarea:not(:placeholder-shown) + label {
      transform: translateY(-.65rem) scale(.85);
      background: color-mix(in oklab, var(--bg-light, #fff), transparent 0%); padding: 0 .25rem; color: var(--accent);
    }
    ...
    .fancy-check input{ appearance:none; width:1.15rem; height:1.15rem; border-radius:.4rem;
      border:1.5px solid color-mix(in oklab, var(--accent), black 30%); display:inline-grid; place-content:center; background:#fff; }
    .dark .fancy-check input{ background:#0b0f1a; border-color: color-mix(in oklab, var(--accent), black 55%) }
    .fancy-check input::before{ content:""; width:.7rem; height:.7rem; transform:scale(0); transition:.12s transform ease-out;
      border-radius:.25rem; background:var(--accent); }
  ...
  {{-- Heading --}}
  <section class="mx-auto mt-[60px] max-w-6xl px-4 text-center">
  ...
  {{-- Form --}}
  <section x-data="{loading:false}" class="mx-auto my-8 max-w-5xl px-4">
    <div class="ring-gradient" id="contactRing">
      <form method="POST" action="{{ route('contacts.submit') }}" class="panel glass">
        @csrf
        ...
        <div class="grid gap-6 p-6 sm:p-8">
          ...
          <div class="fl">
            <input name="subject" value="{{ old('subject') }}" placeholder=" " class="dark:text-white" />
          ...
          <div class="mt-2 flex items-center justify-between gap-3">
            ...
            <button type="submit" @click="loading=true" :disabled="loading" class="btn-accent inline-flex items-center gap-2 rounded-xl px-5 py-3">
  ...
  {{-- reCAPTCHA v3 — attiva quando hai le chiavi
       1) Metti la tua SITE KEY qui sotto
       2) Nel controller verifica il token con la SECRET KEY
  --}}
