{{-- resources/views/public/packs/index.blade.php --}}
<x-app-layout>
  <x-auto-hero />

  {{-- FILTER BAR – sticky + blur + pills --}}
    ...
    <div class="rounded-2xl border border-gray-200/60 bg-white/70 p-2 shadow-sm backdrop-blur
      ...
      <form method="GET" action="{{ route('packs.public') }}"
        ...
        <div class="flex items-center gap-2 sm:col-span-12">
          <button class="inline-flex items-center gap-2 rounded-full bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white
                         hover:opacity-90">
          ...
          <div class="flex flex-wrap items-center gap-2">
            @if($q ?? false)
              <a href="{{ request()->fullUrlWithQuery(['q'=>null]) }}"
                 class="group inline-flex items-center gap-2 rounded-full bg-gray-100 px-3 py-1 text-xs
                        hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700">
                “{{ \Illuminate\Support\Str::limit($q,20) }}”
                <span class="rounded-full bg-gray-300/60 px-1.5 py-0.5 text-[10px] group-hover:bg-gray-400/70">×</span>
              </a>
            @endif
            @if($cat ?? false)
...
@php
  // gruppa i risultati paginati per nome categoria (o “Uncategorized”)
...
@if($packs->count())
  @foreach($groups as $catName => $items)
    @php
      // prendo il “modello” della categoria dal primo elemento del gruppo
      $first   = $items->first();
      $cat     = optional($first)->category;
      $color   = $cat->color ?? 'indigo';

      // mappa tailwind nota; se è HEX uso inline style
      $palette = [
      ...
      $catBadgeClass = $palette[$color] ?? 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-100';
      $catBadgeStyle = '';
